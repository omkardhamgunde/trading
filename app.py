    ...
    while True:
        try:
            with app.app_context():
                ...
                # 1. Get validated current date
                current_date = datetime.now(pytz.utc).date()
                print(f"Current UTC Date: {current_date}")
                
                # 2. Strict date validation
                if current_date.year > datetime.now().year:
                    print("❌ FATAL ERROR: System date in future!")
                    return  # Stop execution completely

                # 3. Calculate real historical range
                end_date = current_date - timedelta(days=1)  # Yesterday
                start_date = end_date - timedelta(days=365)
                
                cursor = mysql.connection.cursor()
                cursor.execute("""
                    SELECT user_id, strat_type, symbol, parameters 
                    FROM strategies 
                    WHERE is_active = 1
                """)
                
                for user_id, strat_type, symbol, params in cursor.fetchall():
                    try:
                        params = json.loads(params)
                        print(f"\nProcessing {symbol} ({strat_type})")

                        # 4. Download validated data
                        ...
                        if latest_date > end_date:
                            print(f"⚠️ Data anomaly: {latest_date} vs {end_date}")
                            continue
                            
                        print(f"Valid data: {data.index[0].date()} to {latest_date}")
                        
                        # SMA Strategy execution
                        if strat_type == 'sma':
                            short = max(10, params.get('short_sma', 50))
                            long = max(50, params.get('long_sma', 200))
                            
                            data['short_sma'] = data['Close'].rolling(short).mean()
                            data['long_sma'] = data['Close'].rolling(long).mean()
                            
                            # Get last 2 valid values
                            short_vals = data['short_sma'].iloc[-2:]
                            long_vals = data['long_sma'].iloc[-2:]
                            
                            if crossover(short_vals, long_vals):
                                print("↑ BUY SIGNAL ↑")
                                execute_trade(user_id, symbol, 'buy', 1)
                            elif crossover(long_vals, short_vals):
                                print("↓ SELL SIGNAL ↓")
                                execute_trade(user_id, symbol, 'sell', 1)
                            else:
                                print("➖ No signal")
...
# Start strategy checker in background
Thread(target=strategy_checker, daemon=True).start()
...
def strategies():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute("""
        SELECT id, strat_name, symbol, strat_type, is_active 
        FROM strategies 
        WHERE user_id = %s
    """, (session['user_id'],))
...
def upload_strategy():
    ...
    try:
        ...
        cursor.execute("""
            ...
            (user_id, strat_name, strat_type, symbol, parameters)
            VALUES (%s, %s, %s, %s, %s)
        """, (
            session['user_id'],
            f"{data['strategy']['type']} Strategy",
            data['strategy']['type'],
            data['symbol'],
        ...
        flash(f'Error uploading strategy: {str(e)}', 'danger')
    
    return redirect('/strategies')

@app.route('/toggle_strategy/<int:strat_id>')
def toggle_strategy(strat_id):
    if 'user_id' not in session:
        return redirect('/login')
